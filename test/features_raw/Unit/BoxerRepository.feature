...
Feature: Boxer Repository Unit Feature
  ...
  Scenario Outline: Edit details of a boxer in Boxer Repository failure
    #Set up mock Repository or real user service -> Mock if @Unit, real if @Integration
    Given there is a boxer such as "<boxer>"
    #Repository.repository_function()
    When "<repository_function>" is invoked with "<edit_body>"
  ...
  Scenario Outline: Remove a boxer from Boxer Repository failure
    ...
    Examples:
      | boxer                                 | repository_function | boxer_id                                 | expected_data                                 |
      | Unit_Repository_Scenario8_Fail1.boxer | removeBoxerWithId   | Unit_Repository_Scenario8_Fail1.boxer_id | Unit_Repository_Scenario8_Fail1.expected_data |
