...
hiptest-publisher --config-file test/hiptest-publisher.conf --test-run-id 546826 --only=features
...
hiptest-publisher --config-file test/hiptest-publisher.conf --push test/results.json  --test-run-id 546826 --push-format cucumber-json
end=$(date +%s)
echo $(($end-$start))
